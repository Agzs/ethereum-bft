...
var (
	...
	DefaultGasLimit uint64 = 21000 // the gas of ether tx should be 21000
)

func SendEther(client client.Client, from *ecdsa.PrivateKey, to common.Address, amount *big.Int, nonce uint64) error {
	tx := types.NewTransaction(nonce, to, amount, DefaultGasLimit, big.NewInt(DefaultGasPrice), []byte{})
